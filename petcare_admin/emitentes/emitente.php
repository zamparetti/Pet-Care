...
<head>
    ...
    <title>PetCare Admin - Emitentes</title>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                <div class="container-fluid">
                    ...
                    <h1 class="h3 mb-2 text-gray-800">Gerenciamento de Emitentes</h1>

                    <form action="" method="post" class="mb-4">
                        <div class="input-group">
                            <input type="text" name="busca" id="busca" class="form-control" placeholder="Nome ou CNPJ">
                    ...
                    <a href="adicionar.php" class="btn btn-success mb-3">Novo Emitente</a>
                                ...
                                <table class="table table-bordered" width="100%" cellspacing="0">
                                    ...
                                    <tbody>
                                        ...
                                        if (!empty($_POST['busca'])) {
                                            $buscar = "%" . $_POST['busca'] . "%";
                                            $sql = $conn->prepare("SELECT * FROM tb_emitente WHERE nm_emitente LIKE :buscar OR cnpj_emitente LIKE :buscar");
                                            $sql->bindParam(':buscar', $buscar, PDO::PARAM_STR);
                                        } else {
                                            $sql = $conn->prepare("SELECT * FROM tb_emitente");
                                        }
                                        $sql->execute();
                                        $result = $sql->fetchAll();
                                        ...
                                        foreach ($result as $linha) {
                                            echo "<tr>";
                                            echo "<td>" . htmlspecialchars($linha['id_emitente']) . "</td>";
                                            echo "<td>" . htmlspecialchars($linha['nm_emitente']) . "</td>";
                                            echo "<td>" . htmlspecialchars($linha['cnpj_emitente']) . "</td>";
                                            ...
                                            echo "<td class='text-center'><a href='editar.php?id=" . htmlspecialchars($linha['id_emitente']) . "' class='btn btn-warning'>Editar</a></td>";
                                            echo "<td class='text-center'><a href='excluir.php?id=" . htmlspecialchars($linha['id_emitente']) . "' class='btn btn-danger' onclick='return confirm(\"Deseja excluir?\");'>Excluir</a></td>";
                                            echo "</tr>";
                                        }
                                        ?>
                                    </tbody>
